...
que es necesaria también. El atributo ticket se mantiene ya que no es un 
identificador único del pasajero (680 valores únicos contra 889 tomados) y 
puede contener información de la localización 
de los pasajeros en el barco.
...
Por ultimo, se convierten las clases con valor categorico a factor, para su 
...
```{r echo=TRUE, message=FALSE, warning=FALSE}
nrow(people_red.male)
nrow(people_red.female)
nrow(people_red.first)
nrow(people_red.second)
nrow(people_red.third)
nrow(people_red.fallecidos)
nrow(people_red.supervivientes)

```

## Comprobación de normalidad y homogeneidad

se analiza si la variables Fare y Age siguen una distribución normal. Para ello 
...
normalidad. Se estudia la homocedasticidad según varios grupos: la clase del 
pasajero (Pclass), el lugar de embarque (Embarked), edad (Age) y el sexo (Sex).
...
Para las cuatro pruebas se obtiene un p-valor inferior al nivel de 
significancia (< 0,05). Por lo tanto, se rechaza la hipótesis nula de homocedasticidad 
y se concluye que la variable Fare presenta varianzas estadísticamente diferentes 
para los diferentes grupos de Pclass, Embarked, Age y Sex.
...
se tendrán en cuenta todos los atributos excepto Survived y Ticket, ya que la variable como se observará, incluir la variable ticket ofrece peores resultados.
```{r echo=TRUE, message=FALSE, warning=FALSE}
library(cluster)
...
gower_dist.t <- daisy(people_red[ , -which(names(people_red) %in% c("Survived"))],
                    metric = "gower",
                    type = list(logratio = 3))

#Calculo de métrica de distancia
gower_dist <- daisy(people_red[ , -which(names(people_red) %in% c("Survived", "Ticket"))],
                    metric = "gower",
                    type = list(logratio = 3))
```
...
td.t <- c(NA)
for(i in 2:30){
  pam_fit.t <- pam(gower_dist.t,
                 diss = TRUE,
                 k = i)
  sil_width.t[i] <- pam_fit.t$silinfo$avg.width
  td.t[i] <- pam_fit.t$objective
  
  # Calculo sin la variable ticket
...
plot(1:30, sil_width.t, type="l",
     xlab = "Agrupaciones",
     ylab = "Silueta (utilizando Ticket)")
lines(1:30, sil_width.t)

plot(1:30, sil_width, type="l",
     xlab = "Agrupaciones",
     ylab = "Silueta")
...
conforman a continuación los 12 grupos. Asimismo, se obtienen mejores resultados sin incluir la variable Ticket.
```{r echo=TRUE, message=FALSE, warning=FALSE}
library(dplyr)
...
head(pam_results$the_summary, n=15)
```
...
arboles de decisión. Para ello se seleccionan las variables comentadas anteriormente 
y se generan las muestras de test y validación.
```{r echo=TRUE, message=FALSE, warning=FALSE}
library(caret)
library(party)
library(partykit)
...
ctree_ <- train(Survived ~ ., data = red_mod, method = "ctree", trControl=train_control)

forest.mod <- train(Survived ~ ., data = red_mod, method = "rf", trControl=train_control)
```
...
cuantitativas como cualitativas. Como no se dispone de conjunto de test, se realiza 
validación cruzada K-fold sobre el modelo de regresión con 10 iteraciones, como 
se ha realizado en el apartado anterior. Para saber que covariables insertar al 
modelo, se debe observar si hay variables de confusión, esto es, variables que 
al tomarlas en cuenta en el modelo, cambien significativamente el factor de otras.

```{r echo=TRUE, message=FALSE, warning=FALSE}
logist <- train(Survived ~ .,
               data = people_red[ , -which(names(people_red) %in% c("Ticket"))],
...
Se observa como las variables de PClass, Sex, Age, SibSp y Parch son 
...
Sobre todo, el modelo identifica bien los registros positivos ya que tiene 
...
Como las varianzas son iguales, se realiza el test para heterocedasticidad para 
determinar si la media de edad de la clase 1a es mayor que la de la 3a.
```{r echo=TRUE, message=FALSE, warning=FALSE}
...
precios de primera clase son más caros, al disponer de una valor p tan pequeño, tal
como se esperaba.
******
# Conclusiones
******
